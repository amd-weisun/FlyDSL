...
def build_fused_rope_cache_module(
    ...
    # VEC_WIDTH: elements per thread. Minimum 1 to keep all threads active.
    ...
    VEC_WIDTH = max(1, head_dim // WARP_SIZE)
    ...
    # Select the right BufferCopy atom based on copy_bits
    def _get_buffer_copy_atom():
        if copy_bits == 16:
    ...
    def fused_qk_rope_reshape_and_cache(
        ...
        # --- Layout API setup ---
        copy_atom = fx.make_copy_atom(_get_buffer_copy_atom(), elem_bits)
        ...
        def ds_bpermute_pair(vec_val, pair_byte_addr):
            ...
            if VEC_WIDTH == 1:
                ...
                v1i16 = vector.bitcast(T.vec(1, T.i16), vec_val)
                ...
                # VEC_WIDTH=2: vector<2xf16/bf16> (32 bits) → vector<1xi32> → scalar i32
                v1i32 = vector.bitcast(T.vec(1, T.i32), vec_val)
                i32_val = vector.extract(v1i32, static_position=[0], dynamic_position=[])
                # Cross-lane shuffle: get pair thread's packed 2-elem value as i32
                peer_i32 = fx.rocdl.ds_bpermute(T.i32, pair_byte_addr, i32_val)
                # Reconstruct vector<1xi32> then bitcast back to vector<2xelem_type>
                peer_v1i32 = vector.from_elements(T.vec(1, T.i32), [peer_i32])
                return vector.bitcast(T.vec(2, elem_type), peer_v1i32)
            ...
            if head_idx < fx.Int32(num_kv_heads_val):
                ...
                if slot_val >= fx.Int32(0):
                    ...
                    if apply_scale:
                        ...
                        ks_rsrc = buffer_ops.create_buffer_resource(KScale, max_size=True)
                        vs_rsrc = buffer_ops.create_buffer_resource(VScale, max_size=True)
                        k_scale_val = buffer_ops.buffer_load(ks_rsrc, fx.Int32(0), vec_width=1, dtype=T.f32)
                        v_scale_val = buffer_ops.buffer_load(vs_rsrc, fx.Int32(0), vec_width=1, dtype=T.f32)
